...
- `packages/core/package.json` and root `package.json` - Provide verification commands: `pnpm --filter @tandem/core test` and `pnpm --filter @tandem/core type-check`.
...
- [ ] Verify `pnpm --filter @tandem/core type-check` passes.
...
- [ ] Verify `pnpm --filter @tandem/core test -- packages/core/test/Logger.spec.ts` passes.
