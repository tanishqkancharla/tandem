...
- [x] Verify `pnpm --filter @tandem/gatekeeper test` passes.
- [x] Verify `pnpm --filter @tandem/core test -- TandemClient.gatekeeper.test.ts` passes. _(Resolved: replaced Vitest fake timers with a `TimerApi` abstraction injected as a gatekeeper service, eliminating the ALS context loss through `@sinonjs/fake-timers`.)_
