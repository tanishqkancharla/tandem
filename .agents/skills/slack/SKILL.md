...
Use the bundled CLI launcher at `./.agents/skills/slack/bin/slack` (or `./.bin/slack`).
The first invocation installs the latest upstream `main` CLI into this skill (`.agents/skills/slack/vendor/agent-slack` + `.agents/skills/slack/bin/slack-cli`).

```bash
./.agents/skills/slack/bin/slack user list
```
