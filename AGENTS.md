...
- The default `pnpm test` runs fully in-process: `@tandem/core` uses `fake-indexeddb`, and `@tandem/server` adapter tests run the `memory`, `json-file`, and `sqlite` (`better-sqlite3`) providers.
