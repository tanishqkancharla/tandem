...
describe("TandemClient", () => {
	tandemClientTest(
		"creates, queries, updates, and deletes records locally",
		async ({ client1 }) => {
	...
	tandemClientTest(
		"runs flat queries unchanged when constructed with a runtime schema",
		async ({ makeClient }) => {
	...
	tandemClientTest(
		"keeps subscribed query results live until the caller unsubscribes",
		async ({ client1 }) => {
	...
	tandemClientTest(
		"lets a caller inspect draft changes and cancel them before commit",
		async ({ client1 }) => {
	...
	tandemClientTest(
		"syncs a committed change from one client to another subscribed client",
	...
	tandemClientTest(
		"syncs remote updates that move records out of subscribed where filters",
		async ({ client1, client2 }) => {
			const seenByClient2: TestsTodo[][] = []
			client2.subscribe(
	...
	tandemClientTest(
		"rolls back an optimistic write when the server rejects the push",
		async ({ makeClient }) => {
			const client = await makeClient({
	...
	tandemClientTest(
		"reloads persisted records after recreating the app",
		async ({ makeClient }) => {
	...
	tandemClientTest(
		"reloads persisted records through schema-owned codecs",
		async ({ logger, rng }) => {
	...
	tandemClientTest(
		"keeps explicit IndexedDB codec support without a runtime schema",
		async ({ logger, rng }) => {
