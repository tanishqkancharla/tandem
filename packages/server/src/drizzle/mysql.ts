...
export type MySqlTableWithId = MySqlTable & DrizzleTableWithId<AnyMySqlColumn>
...
export type MySqlDrizzleRemoteArgs<Schema extends AnySchema> = {
	...
	tables: {
		readonly [Collection in CollectionName<Schema>]: MySqlTableWithId
	}
}
...
> implements RemoteStore<Schema> {
	...
	private async setRecord<Collection extends CollectionName<Schema>>(
		...
		await db
			...
			.onDuplicateKeyUpdate({ set: value as typeof table.$inferInsert })
	}
